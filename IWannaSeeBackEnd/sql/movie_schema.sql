...
create table if not exists `movie_category`(
  ...
  foreign key(`movie_id`) references `movie`(`user_id`) on delete cascade,
  foreign key(`category_id`) references `category`(`user_id`) on delete cascade
);
...
from `movie_category` as MC inner join `category` as C on MC.category_id = C.user_id
group by `movie_id`;
